...
- **Day 17, price-aware dynamic sell-throttling:** replaced the flat
  per-turn sell caps (guessed numbers like "MELON: 2") with
  `dynamic_sell_quantity`, which reacts to the ACTUAL current market
  price relative to each good's base price. Below 40% of base, sell
  much less regardless of category - even a "safe" staple grinds its
  own price further down if dumped while already depressed. Above 75%
  of base, a throttled good can sell up to double its normal cap, since
  the price has room to absorb it. Verified with unit tests across
  healthy/crashed/mid-range price scenarios - all pass. **Important
  caveat: the local flat-price simulator can't meaningfully validate
...
- **Day 21, melon pushed further, one false alarm resolved:** given two
  ...
  fix needed. Real-engine seeded diagnostic confirmed $28,696 vs
  the $23,550 Day 20 baseline (+21.8%), submitted.

- **Day 22, further melon push and surplus fertilizer:** melon raised
  from 7 to 9. Also allowed wheat and carrot to use fertilizer, but
  ONLY from the free surplus generated by animals (`COLLECT_FERTILIZER`).
  They never trigger a market purchase for fertilizer, reserving that
  cash for melon only. Verified locally, then tested against real engine
  with fixed seed.
  Result: $28,761 vs $28,696 baseline. A small but positive (+ $65) gain
...
Day 22 pushed melon to 9 and enabled surplus fertilizer for wheat/carrot
(confirmed $28,761 vs $28,696). Land expansion remains off - settled dead
end per Day 16. **Confirmed against real engine - submitted.**
...
- [x] Day 20: free fertilizer collection (was entirely unclaimed before)
      ...
      submission (474.1)
- [x] Day 21: melon tile target 5->7, building on two consecutive
      confirmed wins in that direction - real-engine CONFIRMED $28,696
