    ...
    for turn in range(turns):
        ...
        for i in range(len(positions)):
            ...
            if act in ("NORTH", "SOUTH", "EAST", "WEST"):
                ...
                if tile is None:
                    tiles[y][x] = {
                        "kind": "PASTURE", "animal": None, "placed_day": day,
                        "yield_units": 0, "fed_today": False, "consecutive_unfed": 0,
                        "cared_today": False, "fertilizer_available": False,
                        "pending_care_bonus": 0,
                    }
                ...
                item = act_list[1]
                if inventories[i].get(item, 0) > 0 and isinstance(tile, dict) and tile.get("kind") in ("COOP", "PASTURE") and tile.get("animal") is None:
